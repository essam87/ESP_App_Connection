...
class Esp32Provider with flutter.ChangeNotifier {
            ...
            _webSocketSubscription = dataStream.listen(
              (data) {
                ...
                if (data.containsKey('type') && data['type'] == 'status') {
                  ...
                  // Update status timestamp for status messages specifically
                  _updateState(lastStatusTimestamp: DateTime.now());
                  ...
                  errorMessage:
                      () =>
                          (_state.errorMessage?.contains('error') ?? false)
                              ...
                              : null, // Don't show "WebSocket disconnected" message
                );
                disconnectWebSocket(); // Ensure cleanup
  ...
  void disconnectWebSocket() {
    ...
    // First cancel the subscription to prevent any callback race conditions
    if (_webSocketSubscription != null) {
      _webSocketSubscription
          ...
          .catchError((e) {
            if (flutter.kDebugMode) {
              flutter.debugPrint('Error canceling subscription: $e');
            }
          });
    }

    // Then disconnect the WebSocket service
    Future.microtask(() {
