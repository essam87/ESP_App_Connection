...
class Esp32Service {
  ...
  void disconnectWebSocket() {
    ...
    // Store local references to avoid null issues during async operations
    ...
    // Close resources in a try-catch to ensure complete cleanup
    Future.microtask(() {
      try {
        // Close the WebSocket sink with a normal closure code
        channel?.sink.close(status.normalClosure).catchError((e) {
          if (kDebugMode) {
        ...
        if (kDebugMode) {
          debugPrint('Exception during sink close: $e');
        }
      }
      ...
      try {
        ...
        if (streamController != null && !streamController.isClosed) {
          streamController.close().catchError((e) {
            if (kDebugMode) {
        ...
        if (kDebugMode) {
          debugPrint('Exception during stream controller close: $e');
        }
      }

      // Reset the disconnecting flag after a small delay
